...
function injectSecurityBarStyles() {
  if (document.getElementById("sec-bar-styles")) return;
  const style = document.createElement("style");
  style.id = "sec-bar-styles";
  style.textContent = `
    .sec-bar {
      position: fixed;
    ...
    .sec-bar-label svg { width: 12px; height: 12px; opacity: 0.5; }
    .sec-bar-back {
      display: inline-flex;
      align-items: center;
      gap: 6px;
    ...
    .sec-bar-back:hover { background: rgba(255,255,255,0.13); color: #fff; }
    .sec-bar-back svg { width: 13px; height: 13px; }
    .sec-bar-logout {
      display: inline-flex;
      align-items: center;
      gap: 7px;
    ...
    .sec-bar-logout:hover { background: #9b2c2c; }
    .sec-bar-logout svg { width: 13px; height: 13px; }
    .nav-warn-overlay {
      position: fixed;
      inset: 0;
      z-index: 99999;
      ...
      animation: secOverlayIn 0.18s ease;
    }
    @keyframes secOverlayIn { from { opacity: 0; } to { opacity: 1; } }
    .nav-warn-card {
      background: #fff;
      border-radius: 18px;
      ...
      animation: secCardIn 0.22s cubic-bezier(0.34,1.56,0.64,1);
    }
    @keyframes secCardIn { from { transform: scale(0.92); opacity: 0; } to { transform: scale(1); opacity: 1; } }
    .nav-warn-icon {
    ...
    .nav-warn-icon svg { width: 24px; height: 24px; color: #c53030; }
    .nav-warn-title { font-size: 1.15rem; font-weight: 800; color: #1a202c; margin-bottom: 10px; }
    .nav-warn-text { font-size: 0.88rem; color: #4a5568; margin-bottom: 26px; line-height: 1.65; }
    .nav-warn-text strong { color: #c53030; }
    .nav-warn-actions { display: flex; gap: 10px; justify-content: center; }
    .nav-warn-stay {
      padding: 9px 22px;
      border-radius: 9px;
    ...
    .nav-warn-stay:hover { background: #edf2f7; }
    .nav-warn-logout {
      padding: 9px 22px;
      border-radius: 9px;
      border: none;
    ...
    .nav-warn-logout:hover { background: #9b2c2c; }
    .nav-warn-logout svg { width: 13px; height: 13px; }
  `;
  document.head.appendChild(style);
}

function showNavWarningModal() {
  if (document.querySelector(".nav-warn-overlay")) return;
  const overlay = document.createElement("div");
  overlay.className = "nav-warn-overlay";
  overlay.innerHTML = `
    <div class="nav-warn-card">
      <div class="nav-warn-icon">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
          <rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>
        </svg>
      </div>
      <p class="nav-warn-title">Keep your account secure</p>
      <p class="nav-warn-text">Using the browser&rsquo;s back button does not log you out &mdash; your session stays open.<br>Always use <strong>Secure Logout</strong> to properly close your session and protect your account.</p>
      <div class="nav-warn-actions">
        <button class="nav-warn-stay">Stay on page</button>
        <button class="nav-warn-logout">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
            <rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>
          </svg>
          Secure Logout
  ...
  `;
  document.body.appendChild(overlay);
  overlay.querySelector(".nav-warn-stay").addEventListener("click", () => overlay.remove());
  overlay.querySelector(".nav-warn-logout").addEventListener("click", () => {
    overlay.remove();
...
function setupSecurityBar() {
  if (!document.body.classList.contains("dashboard-page")) return;
  const auth = getAuth();
  if (!auth) return;

  injectSecurityBarStyles();

  const page = document.body.dataset.page;
  const isMainDashboard = page === "admin-dashboard" || page === "user-dashboard";
  const dashboardHref = auth.role === "admin" ? "./admin-dashboard.html" : "./user-dashboard.html";

  const bar = document.createElement("div");
  bar.className = "sec-bar";
  bar.innerHTML = `
    <span class="sec-bar-label">
    ...
    ${!isMainDashboard ? `
    <a class="sec-bar-back" href="${dashboardHref}">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        <path d="M19 12H5M12 19l-7-7 7-7"/>
      </svg>
      Back to Dashboard
    </a>` : ""}
    <button class="sec-bar-logout" type="button">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
        <rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>
      </svg>
      Secure Logout
  ...
  bar.querySelector(".sec-bar-logout").addEventListener("click", () => logoutAndResetPortals());
}

function setupBrowserNavGuard() {
  if (!document.body.classList.contains("dashboard-page")) return;
  if (!getAuth()) return;

  history.pushState({ secureSession: true }, "");
