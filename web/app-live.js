  ...
  document.querySelectorAll("[data-edit-investor]").forEach((button) => {
    ...
    button.addEventListener("click", () => {
      showInvestorEditModal({
        ...
        email: button.dataset.editEmail || "",
...
function showInvestorEditModal({ userId, name, phone, email, isActive, initialFunds, balanceFunds }) {
  ...
  overlay.className = "admin-pwd-modal-overlay";
  overlay.innerHTML = `
    <div class="admin-pwd-modal admin-pwd-modal--wide" role="dialog" aria-modal="true" aria-labelledby="investorEditTitle" style="max-width:560px;max-height:90vh;overflow-y:auto;">
      <h3 id="investorEditTitle" style="color:#2c90f0;margin-bottom:4px;">Edit Investor</h3>
      <p style="font-size:0.83rem;color:#6b7a99;margin-bottom:18px;">Changes apply immediately to the database.</p>

      <p style="font-size:0.75rem;font-weight:700;letter-spacing:0.07em;text-transform:uppercase;color:#9aa5b8;margin:0 0 10px;">Profile</p>
      <div class="admin-inline-form-grid" style="margin-bottom:16px;">
        ...
        <div class="admin-form-field">
          <label for="ief_phone">Mobile Number</label>
          <input class="input-field" id="ief_phone" type="tel" value="${escapeHtml(phone)}" />
        </div>
        <div class="admin-form-field">
          <label for="ief_email">Email</label>
          <input class="input-field" id="ief_email" type="email" value="${escapeHtml(email)}" placeholder="optional" />
        </div>
        <div class="admin-form-field">
      ...
      <p style="font-size:0.75rem;font-weight:700;letter-spacing:0.07em;text-transform:uppercase;color:#9aa5b8;margin:0 0 10px;">Password</p>
      <div class="admin-inline-form-grid" style="margin-bottom:16px;">
        <div class="admin-form-field">
          <label for="ief_password">New Password <span style="font-weight:400;color:#9aa5b8;">(leave blank to keep current)</span></label>
          <input class="input-field" id="ief_password" type="password" placeholder="Min 6 characters" autocomplete="new-password" />
        </div>
      </div>
      ...
      <div class="admin-inline-form-grid" style="margin-bottom:16px;">
        ...
        <div class="admin-form-field">
          <label for="ief_remove_fund">Remove Funds (₹)</label>
          <input class="input-field" id="ief_remove_fund" type="number" min="0" step="0.01" value="0" placeholder="Amount to deduct" />
        </div>
      </div>
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    const balance_funds = Math.max(0, balanceFunds + add_fund - remove_fund);

    if (!full_name || !phone_number) {
      errEl.textContent = "Name and mobile number are required."; errEl.style.display = "block"; return;
    }
    ...
    try {
      ...
      showAdminSuccessModal("Investor Updated", [
        ...
        ["New Balance", `₹${balance_funds.toLocaleString("en-IN")}`],
        ...(new_password ? [["Password", "Reset successfully"]] : []),
      ]);
      setTimeout(() => renderAdminDatabasePage({ silent: true }), 900);
    } catch (err) {
      errEl.textContent = formatError(err); errEl.style.display = "block";
                            ...
                            <tr>
                              ...
                              <td style="display:flex;gap:6px;">
                                <button class="secondary-btn compact-btn" type="button"
                                  ...
                                  data-edit-email="${escapeHtml(user.email || '')}"
...
function _openReportWindow(html, filename) {
...
${wmHtml}
<div class="hdr"><div><div class="brand">Asset Yantra</div><div class="brand-tag">Markets. Insights. Wealth.</div></div><div><div class="rpt-title">Portfolio Report</div><div class="rpt-date">${reportDate} &nbsp;·&nbsp; ${reportTime}</div></div></div>
...
${wmHtml}
<div class="hdr"><div><div class="brand">Asset Yantra</div><div class="brand-tag">Markets. Insights. Wealth.</div></div><div><div class="rpt-title">Stock Investor Report</div><div class="rpt-date">${reportDate} &nbsp;·&nbsp; ${reportTime}</div></div></div>
  ...
  document.querySelectorAll("[data-user-detail]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (_pdfBtn) _pdfBtn.addEventListener("click", async () => {
        printClientReport(user, soldHistory);
      });
    ...
    button.addEventListener("click", () => {
      ...
      if (user) {
        ...
        if (_spdfBtn) _spdfBtn.addEventListener("click", async () => {
          printStockReport(symbol, holdings);
        });
      }
      detailMount.classList.remove("hidden");
      detailMount.classList.add("portal-visible");
...
(function () {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    try {
      ...
      status.textContent = "Could not send. Please WhatsApp or email us directly.";
      status.style.color = "#c62828";
    }
    btn.disabled = false;
...
// ── Trust & Safety — mouse glow + scroll reveal ────────────────────────────
(function () {
  const section = document.getElementById("capitalSafety");
  const glow = document.getElementById("csMouse");
  if (section && glow) {
  ...
  var io = new IntersectionObserver(function (entries) {
    entries.forEach(function (entry) {
      if (entry.isIntersecting) {
        var el = entry.target;
        var idx = parseInt(el.dataset.csIndex || "0", 10);
        setTimeout(function () { el.classList.add("cs-card-visible"); }, idx * 160);
        io.unobserve(el);
      }
    });
  }, { threshold: 0.15 });
  document.querySelectorAll(".cs-card").forEach(function (c) { io.observe(c); });
  var headerIo = new IntersectionObserver(function (entries) {
    entries.forEach(function (e) { if (e.isIntersecting) { e.target.classList.add("cs-header-visible"); headerIo.unobserve(e.target); } });
  }, { threshold: 0.3 });
  var hdr = document.querySelector(".cs-header");
  if (hdr) headerIo.observe(hdr);
})();

// ── Forex — Algo live data renderer ───────────────────────────────────────
(function () {
  if (!document.getElementById("algoPerformanceMount")) return;
  const STATIC_ACCOUNTS = [
    {
      id: "global-algo-3", name: "Global Algo 3", invested: 1500, current: 2488, profit: 988, profitPct: 65.9,
      period: "Feb 2026 to May 2026", totalTrades: 2483, winRate: 51.9,
      weeks: [
        { label: "9 - 13 Feb 26", trades: 112, winPct: 49.1, profit: 115, roi: 7.7 },
        { label: "16 - 20 Feb 26", trades: 193, winPct: 53.9, profit: 149, roi: 9.9 },
        { label: "23 - 27 Feb 26", trades: 204, winPct: 54.4, profit: 71, roi: 4.8 },
        { label: "2 - 6 Mar 26", trades: 348, winPct: 52.0, profit: 153, roi: 10.2 },
        { label: "9 - 13 Mar 26", trades: 399, winPct: 52.6, profit: 133, roi: 8.8 },
        { label: "16 - 20 Mar 26", trades: 252, winPct: 52.0, profit: 60, roi: 4.0 },
        { label: "23 - 27 Mar 26", trades: 150, winPct: 53.3, profit: 73, roi: 4.9 },
        { label: "30 Mar - 3 Apr 26", trades: 301, winPct: 49.5, profit: 83, roi: 5.5 },
        { label: "6 - 10 Apr 26", trades: 116, winPct: 52.6, profit: 95, roi: 6.3 },
        { label: "13 - 17 Apr 26", trades: 112, winPct: 49.1, profit: -24, roi: -1.6 },
        { label: "20 - 24 Apr 26", trades: 120, winPct: 49.2, profit: 25, roi: 1.7 },
        { label: "27 Apr - 1 May 26", trades: 64, winPct: 53.1, profit: 13, roi: 0.9 },
        { label: "4 - 8 May 26", trades: 112, winPct: 51.8, profit: 43, roi: 2.9 }
      ]
    },
    {
      id: "global-algo-2", name: "Global Algo 2", invested: 5000, current: 10821, profit: 5821, profitPct: 116.4,
      period: "Dec 2025 to May 2026", totalTrades: 3302, winRate: 53.3,
      weeks: [
        { label: "29 Dec 25 - 2 Jan 26", trades: 18, winPct: 38.9, profit: 49, roi: 1.0 },
        { label: "5 - 9 Jan 26", trades: 898, winPct: 56.1, profit: 573, roi: 11.5 },
        { label: "12 - 16 Jan 26", trades: 174, winPct: 50.6, profit: 48, roi: 1.0 },
        { label: "19 - 23 Jan 26", trades: 72, winPct: 54.2, profit: 100, roi: 2.0 },
        { label: "26 - 30 Jan 26", trades: 158, winPct: 52.5, profit: 468, roi: 9.4 },
        { label: "9 - 13 Feb 26", trades: 466, winPct: 53.4, profit: 771, roi: 15.4 },
        { label: "16 - 20 Feb 26", trades: 222, winPct: 50.9, profit: 922, roi: 18.4 },
        { label: "23 - 27 Feb 26", trades: 221, winPct: 54.3, profit: 528, roi: 10.6 },
        { label: "2 - 6 Mar 26", trades: 155, winPct: 53.5, profit: 418, roi: 8.4 },
        { label: "9 - 13 Mar 26", trades: 194, winPct: 53.1, profit: 256, roi: 5.1 },
        { label: "16 - 20 Mar 26", trades: 188, winPct: 50.5, profit: 505, roi: 10.1 },
        { label: "23 - 27 Mar 26", trades: 88, winPct: 52.3, profit: 134, roi: 2.7 },
        { label: "30 Mar - 3 Apr 26", trades: 154, winPct: 51.9, profit: 362, roi: 7.2 },
        { label: "6 - 10 Apr 26", trades: 64, winPct: 50.0, profit: 296, roi: 5.9 },
        { label: "13 - 17 Apr 26", trades: 62, winPct: 53.2, profit: 93, roi: 1.9 },
        { label: "20 - 24 Apr 26", trades: 62, winPct: 51.6, profit: 109, roi: 2.2 },
        { label: "27 Apr - 1 May 26", trades: 50, winPct: 50.0, profit: 72, roi: 1.4 },
        { label: "4 - 8 May 26", trades: 56, winPct: 50.0, profit: 116, roi: 2.3 }
      ]
    }
  ];
  function fmtUSD(n) {
  ...
  function buildBarChart(weeks) {
    ...
    const chartMax = Math.max(...profits, 1);
    const tickStep = chartMax > 500 ? 250 : chartMax > 100 ? 50 : 25;
    const topTick = Math.ceil(chartMax / tickStep) * tickStep;
    const ticks = [];
    for (let t = topTick; t >= 0; t -= tickStep) ticks.push(t);
  ...
  function buildAccountCard(acct) {
    return `<article class="algo-account-card"><div class="algo-account-header"><h3>${acct.name}</h3><span class="algo-account-period">${acct.period}</span></div><div class="algo-card-body"><div class="algo-journey"><div class="algo-journey-item"><span class="algo-journey-label">Invested</span><strong class="algo-journey-val">${fmtUSD(acct.invested)}</strong></div><div class="algo-journey-arrow">→</div><div class="algo-journey-item"><span class="algo-journey-label">Current Value</span><strong class="algo-journey-val algo-journey-now">${fmtUSD(acct.current)}</strong></div></div><p class="algo-profit-headline"><strong class="profit">${fmtUSD(acct.profit)} net profit (+${acct.profitPct}%)</strong> over this period</p><div class="algo-stat-strip"><div class="algo-stat-item"><strong>${acct.totalTrades.toLocaleString()}</strong><span>Total Trades</span></div><div class="algo-stat-item"><strong>${acct.winRate}%</strong><span>Win Rate</span></div><div class="algo-stat-item profit"><strong>${fmtUSD(acct.profit)}</strong><span>Net Profit</span></div><div class="algo-stat-item"><strong>${fmtUSD(acct.invested)}</strong><span>Invested</span></div><div class="algo-stat-item profit"><strong>${fmtUSD(acct.current)}</strong><span>Portfolio Value</span></div></div><p class="algo-chart-label">Weekly Profit Performance</p>${buildBarChart(acct.weeks)}<p class="algo-table-section-label">Week-by-Week Breakdown</p><div class="algo-table-wrap"><table class="algo-perf-table"><thead><tr><th>Week Range</th><th>Trades</th><th>Win %</th><th>Profit</th><th>ROI</th></tr></thead><tbody>${buildTableRows(acct.weeks)}</tbody></table></div></div></article>`;
  }
  function renderAccounts(accounts) {
    const mount = document.getElementById("algoPerformanceMount");
    if (!mount) return;
    mount.innerHTML = accounts.map(buildAccountCard).join("");
    initForexCountUp();
  }
  function initForexCountUp() {
    const elements = document.querySelectorAll(".algo-journey-val, .algo-stat-item strong");
    const parseNum = str => { const clean = str.replace(/[$,+%]/g, "").trim(); return isNaN(parseFloat(clean)) ? null : parseFloat(clean); };
    const formatLike = (original, val) => {
      if (original.startsWith("$")) return "$" + Math.round(val).toLocaleString("en-US");
      if (original.endsWith("%")) return val.toFixed(1) + "%";
      if (original.includes(",")) return Math.round(val).toLocaleString("en-US");
    ...
    const animate = el => {
      ...
      const tick = now => { const p = Math.min((now - start) / dur, 1); el.textContent = formatLike(original, target * ease(p)); if (p < 1) requestAnimationFrame(tick); else el.textContent = original; };
      requestAnimationFrame(tick);
    };
    if (!("IntersectionObserver" in window)) { elements.forEach(animate); return; }
  ...
  async function tryLiveData() {
    try {
      ...
      renderAccounts(STATIC_ACCOUNTS);
    }
  }
